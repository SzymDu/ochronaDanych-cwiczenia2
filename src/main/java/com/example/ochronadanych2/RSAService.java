...
public class RSAService {
    ...
    public static String encode(RSA rsa) {
        ...
        String[] messageArray = rsa.getMessage().split(" ");
        for (int i = 0; i < messageArray.length; i++) {
            String[] split = messageArray[i].split("");
            List<String> splittetMessage = Arrays.stream(split)
                    .collect(Collectors.toList());
            if(splittetMessage.size() % 2== 1 && i == messageArray.length-1){
            ...
            for (int j = 0; j < splittetMessage.size(); j++) {
                BigInteger tmp = BigInteger.valueOf(splittetMessage.get(j).charAt(0));
    ...
    public static String decode(RSA rsa) {
        ...
        return message.toString().replaceAll("0", "");
    }
   ...
   public static BigInteger pow(BigInteger base, BigInteger exponent) {
    ...
    public static void main(String[] args) {
        String message = encode(RSA.builder().message("Kok").keyIndex(String.valueOf(0)).build());
